...
class KeyStringLocalizedView(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        try:
            ...
            keyvalues_q = get_localized_strings(
                app=app, locale_code=locale_code)
            queryset = keyvalues_q.order_by('key',)

            serializer = KeyValueSerializer(queryset, many=True)
            response = Response(serializer.data)
            last_modified_q = keyvalues_q.order_by(
                '-modified').values_list('modified')

            if last_modified_q.exists():
                x_last_modified = "%s" % (last_modified_q.first()[
                                          0].strftime("%Y-%m-%d %H:%M %Z"))
