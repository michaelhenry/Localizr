...
class KeyString(UserInfoSavableModel):
    ...
    def available_locales(self):
        return ", ".join(list(self.values.values_list(
            'locale__code', flat=True).order_by('locale__code')))
...
def user_app_ids_query(user):
    ...
    if user.is_superuser:
        ...
        q = AppInfo.objects.filter(
            Q(pk__in=group_app_ids) | Q(pk__in=user_app_ids))
